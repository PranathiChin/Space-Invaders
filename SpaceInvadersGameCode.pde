  ...
  for (int i = 0; i < 55; i++) {
    if (alienExists[i]) {
      ...
      if (aliens[i].getExplode() > 0) {
        ...
        if (!over && bullet && bulletX < aliens[i].getX()+taille && bulletX+2 > aliens[i].getX() && bulletY < aliens[i].getY()+16 && bulletY+6 > aliens[i].getY()) {
          aliens[i].explode();
        ...
        if (!over && random(1000) < 0.2) {
          int ind = 0;
          while (bulletExists[ind]) ind++;
          bullets[ind] = new Bullet(aliens[i].getX()+taille/2-1, aliens[i].getY()+16);
          bulletExists[ind] = true;
        }
      }
    }
  }
...
void chargeData() {
  font = createFont("assets/FontToUse.ttf", 20);
